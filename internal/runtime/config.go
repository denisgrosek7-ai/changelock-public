...
func LoadSelfHealingConfig() (SelfHealingConfig, error) {
	return LoadSelfHealingConfigFromEnv(os.Getenv)
}

func LoadSelfHealingConfigFromEnv(getenv func(string) string) (SelfHealingConfig, error) {
	if getenv == nil {
		getenv = os.Getenv
	}
	mode := RemediationMode(strings.ToLower(strings.TrimSpace(envOrDefaultFrom(getenv, "CHANGELOCK_SELF_HEALING_MODE", string(RemediationModeDisabled)))))
	switch mode {
	...
	maxAttempts := 3
	if raw := strings.TrimSpace(getenv("CHANGELOCK_SELF_HEALING_MAX_ATTEMPTS")); raw != "" {
	...
	if raw := strings.TrimSpace(getenv("CHANGELOCK_SELF_HEALING_WINDOW")); raw != "" {
	...
	allowedKinds, err := parseAllowedKinds(getenv("CHANGELOCK_SELF_HEALING_ALLOWED_KINDS"))
	if err != nil {
		return SelfHealingConfig{}, err
	}

	failMode, err := parseFailMode(getenv("CHANGELOCK_CLOSED_LOOP_FAIL_MODE"))
	if err != nil {
		return SelfHealingConfig{}, err
	}
	reconcileInterval, err := parseReconcileInterval(getenv("CHANGELOCK_CLOSED_LOOP_RECONCILE_INTERVAL"))
	if err != nil {
		return SelfHealingConfig{}, err
	}
	protectedNamespaces := parseCSVSet(getenv("CHANGELOCK_CLOSED_LOOP_PROTECTED_NAMESPACES"), []string{"changelock", "changelock-system"})
	protectedWorkloads := parseCSVSet(getenv("CHANGELOCK_CLOSED_LOOP_PROTECTED_WORKLOADS"), nil)
	runtimeVEXSeverity, err := parseRuntimeVEXSeverity(getenv("CHANGELOCK_RUNTIME_VEX_QUARANTINE_SEVERITY"))
	if err != nil {
		return SelfHealingConfig{}, err
	}
	requireSignedDesiredState := parseBoolValue(getenv("CHANGELOCK_CLOSED_LOOP_REQUIRE_SIGNED_DESIRED_STATE"))
	if !requireSignedDesiredState {
		requireSignedDesiredState = parseBoolValue(getenv("CHANGELOCK_SELF_HEALING_REQUIRE_SIGNED_DESIRED_STATE"))
	}
	...
	return SelfHealingConfig{
		...
		CriticalOnly:              parseBoolValue(getenv("CHANGELOCK_SELF_HEALING_CRITICAL_ONLY")),
		FailMode:                  failMode,
		VerifyDesiredStateOnLoop:  parseBoolValue(getenv("CHANGELOCK_CLOSED_LOOP_VERIFY_DESIRED_STATE_ON_RECONCILE")),
		ReconcileInterval:         reconcileInterval,
		ProtectedNamespaces:       protectedNamespaces,
		ProtectedWorkloads:        protectedWorkloads,
		QuarantineNetworkPolicy:   parseBoolValue(getenv("CHANGELOCK_RUNTIME_QUARANTINE_NETWORK_POLICY_ENABLED")),
		RuntimeVEXQuarantine:      parseBoolValue(getenv("CHANGELOCK_RUNTIME_VEX_QUARANTINE_ENABLED")),
		RuntimeVEXSeverity:        runtimeVEXSeverity,
		RuntimeVEXRequireNet:      !strings.EqualFold(strings.TrimSpace(getenv("CHANGELOCK_RUNTIME_VEX_QUARANTINE_REQUIRE_NET_ACTIONABLE")), "false"),
	}, nil
}
...
func envOrDefault(key, fallback string) string {
	return envOrDefaultFrom(os.Getenv, key, fallback)
}

func envOrDefaultFrom(getenv func(string) string, key, fallback string) string {
