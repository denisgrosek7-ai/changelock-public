...
func point13ValETwitterFutureDated(entries []Point13ValDTimelineEvent, verifiedAt string) bool {
	for _, entry := range entries {
		if strings.TrimSpace(entry.CanonicalOccurredAt) > strings.TrimSpace(verifiedAt) {
			return true
		}
	}
	return false
...
func point13ValEPassClosureManifestStateAndReasons(model Point13PassClosureManifest, foundation Point13ValEFoundation, expectedPassAllowed bool) (string, []string) {
	...
	if strings.TrimSpace(model.DependencyGateResult) != Point13ValEStateActive ||
		strings.TrimSpace(model.ClosureEvaluatorResult) != Point13ValEStateActive ||
		strings.TrimSpace(model.NoOverclaimResult) != Point13ValEStateActive ||
		strings.TrimSpace(model.AuthorityBoundaryResult) != Point13ValEStateActive ||
		strings.TrimSpace(model.TimestampIntegrityResult) != Point13ValEStateActive ||
		strings.TrimSpace(model.TenantIsolationResult) != Point13ValEStateActive ||
		strings.TrimSpace(model.EvidenceIntegrityResult) != Point13ValEStateActive ||
	...
	switch strings.TrimSpace(model.ReviewerResult) {
	case point12ValEReviewerResultPassConfirmed:
		if !expectedPassAllowed || !model.Point13PassAllowed || strings.TrimSpace(model.Point13PassToken) != point13ValEPoint13PassToken {
	...
	case point12ValEReviewerResultReviewRequired:
		if model.Point13PassAllowed || strings.TrimSpace(model.Point13PassToken) != "" {
			reasons = append(reasons, "pass_closure_manifest_review_required_emits_point13_pass")
		}
		return Point13ValEStateReviewRequired, append(reasons, "pass_closure_manifest_reviewer_requires_review")
	default:
	...
	if !expectedPassAllowed && strings.TrimSpace(model.Point13PassToken) != "" {
		reasons = append(reasons, "pass_closure_manifest_token_present_before_final_happy_path")
	}
	if !point13ValEPassCandidate(foundation) {
